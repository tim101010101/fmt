...
pub(crate) use lex::{type_judgument, LexedToken, TokenStream};
...
mod tests {
    ...
    fn smoke_test() {
        ...
        sayHello(msg);
        "#);
        let function_decla = Box::new(stat_node(FunctionDeclaStatement {
            ...
            body: vec![Box::new(expr_node(FunctionCallExpr {
                ...
                args: vec![Box::new(expr_node(BinaryExpr {
                    kind: BINARY_EXPR,
                    left: Box::new(literal_node(Id {
                        kind: ID,
                        name: "value".to_string(),
                    })),
                    op: PLUS,
                    right: Box::new(literal_node(StringLiteral {
                        kind: STRING,
                        value: " 2022.11.4".to_string(),
                        raw: "\" 2022.11.4\"".to_string(),
                    })),
                }))],
            }))],
        }));
        let variable_decla = Box::new(stat_node(VariableDeclaStatement {
            ...
            init: Box::new(literal_node(StringLiteral {
                kind: STRING,
                value: "Hello FMT".to_string(),
                raw: "\"Hello FMT\"".to_string(),
            })),
        }));
        let function_call = Box::new(expr_node(FunctionCallExpr {
            kind: FUNCTION_CALL_EXPR,
        ...
        assert_eq!(
            ...
            Ok(Root {
                kind: ROOT,
                statements: vec![function_decla, variable_decla, function_call]
            })
        );
    }
}
