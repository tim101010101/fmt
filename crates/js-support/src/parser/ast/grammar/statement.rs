...
{
    right(single_token(CASE_KW), boxed_expr_node())
        .and_then(|expr| {
            switch_stat1().map(
...
mod tests {
    ...
    use crate::parser::ast::Expr::{
        AssignmentExpr, BinaryExpr, UnaryExpr,
    };
    ...
    fn test_stat() {
        ...
        assert_eq!(
            Ok((
                vec![], stat_node(FunctionDeclaStatement {
    kind: FUNCTION_DECLA_STAT,
    name: foo.clone(),
