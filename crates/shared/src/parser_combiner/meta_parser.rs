...
pub fn atom<'input, Input, InputItem>(
) -> impl Parser<'input, Input, InputItem>
where
    InputItem: PartialOrd,
    Input: IntoIterator<Item = InputItem>
        + FromIterator<InputItem>
        + PartialOrd
        + Clone,
{
    ...
    {
        Some(next) => {
            Ok((input.into_iter().skip(1).collect(), next))
        }
        None => Err(input),
    }
}
...
// TODO temporarily use cloning to passing the tests
pub fn literal<'input, Input, InputItem>(
    expect: Input,
) -> impl Parser<'input, Input, Input>
where
    InputItem: PartialOrd,
    Input: IntoIterator<Item = InputItem>
        + FromIterator<InputItem>
        ...
        + Clone,
{
    move |input: Input| {
        ...
        if expect_input == expect {
            Ok((
                input.into_iter().skip(len).collect(),
                expect_input,
            ))
        } else {
            Err(input)
...
pub fn skip_literal<'input, Input, InputItem>(
    expect: Input,
) -> impl Parser<'input, Input, ()>
where
...
mod tests {
    use crate::parser_combiner::meta_parser::{
        literal, single_literal, skip_literal,
        skip_single_literal,
    };
    ...
    fn test_skip_literal() {
        ...
        assert_eq!(
            Ok((vec![B], ())),
            skip_literal(vec![A]).parse(input)
        )
    }
    ...
    fn test_skip_single_literal() {
        ...
        assert_eq!(
            Ok((vec![B], ())),
            skip_single_literal(A).parse(input)
        )
    }
}
